...
module.exports = {
  ...
  run: function (options) {
    ...
    return tabSelector.run().then(tabs => {
      return Promise.all(tabs.map(this._rows.bind(this))).then(rowsByTab => {
  ...
  _clean: function(locales) {
    Object.keys(locales).forEach(locale => {
      filesystem.clean(filesystem.path(locale));
    });
  },
